...
//Modified to include the following productions: TEXT, ISOSCELES, PARALLELOGRAM, REGULAR_POLYGON
...
class Parser {
    ...
    // isosceles -> ISOSCELES COLOR number_list AT number_list HIGHT NUMBER WIDTH NUMBER ';'
    // parallelogram -> COLOR number_list number_list OFFSET NUMBER
    ...
    private void parseImages(Scene scene, Token imageToken) throws LexicalError, SyntaxError, IOException {
        ...
        if (imageToken == Token.RIGHT_TRIANGLE) {
            ...
            verifyNextToken(Token.NUMBER);
            width = lexer.getNumber();
            IsoscelesTriangle iso_triangle = new IsoscelesTriangle(color, point, height, width);
            scene.addImage(iso_triangle);
            ...
            RegularPolygon regular_polygon = new RegularPolygon(color, point, sides, radius);
